defmodule GibberingTales.CampaignInvitationsTest do
  use GibberingTales.DataCase, async: true

  alias GibberingTales.{CampaignInvitations, Campaigns}
  ...
  describe "approve/1 (DM approves player_request)" do
    test "creates CampaignCharacter, joins campaign, marks invitation approved", %{
      ...
      ccs = GibberingTales.CampaignCharacters.list_for_campaign(campaign_id)
      assert length(ccs) == 1
  ...
  describe "reject/1 (DM rejects player_request)" do
    test "marks invitation rejected without creating records", %{
      ...
      assert GibberingTales.CampaignCharacters.list_for_campaign(campaign_id) == []
    end
  end
