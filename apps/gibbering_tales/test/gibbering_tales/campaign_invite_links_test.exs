defmodule GibberingTales.CampaignInviteLinksTest do
  use GibberingTales.DataCase, async: true

  alias GibberingTales.{CampaignInviteLinks, Campaigns}
  alias GibberingTales.AccountsFixtures
  alias GibberingTales.GameFixtures
  ...
  describe "get_by_token/1" do
    ...
    test "returns {:error, :expired} for an expired token", ctx do
      ...
      GibberingTales.Repo.update!(Ecto.Changeset.change(link, expires_at: past))
  ...
  describe "redeem/2" do
    ...
    test "returns {:error, :uses_exhausted} when uses_remaining reaches 0", ctx do
      ...
      {:ok, _} = CampaignInviteLinks.redeem(link, ctx.player.id)
      link = GibberingTales.Repo.get!(GibberingTales.CampaignInviteLink, link.id)
