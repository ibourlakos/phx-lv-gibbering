defmodule Gibbering.Engine.AppearanceArchetypeTest do
  use ExUnit.Case, async: true

  alias Gibbering.Engine.AppearanceArchetype

  # --- archetype_for/2 ---

  describe "archetype_for/2" do
    test "resolves wolf to quadruped via static map" do
      assert AppearanceArchetype.archetype_for("wolf") == :quadruped
    end

    test "resolves rock and chest to structure via static map" do
      assert AppearanceArchetype.archetype_for("rock") == :structure
      assert AppearanceArchetype.archetype_for("chest") == :structure
    end

    test "defaults to biped_upright for unknown sprites" do
      assert AppearanceArchetype.archetype_for("goblin") == :biped_upright
      assert AppearanceArchetype.archetype_for("human_fighter") == :biped_upright
      assert AppearanceArchetype.archetype_for("totally_unknown") == :biped_upright
    end

    test "appearance data 'archetype' key overrides static map" do
      assert AppearanceArchetype.archetype_for("goblin", %{"archetype" => "swarm"}) == :swarm
    end

    test "appearance data archetype overrides static map for wolf too" do
      assert AppearanceArchetype.archetype_for("wolf", %{"archetype" => "biped_upright"}) ==
               :biped_upright
    end
  end
  ...
  describe "canonical_facing/1" do
    test "west maps to east with flip" do
      assert AppearanceArchetype.canonical_facing(:west) == {:east, true}
    end

    test "other facings are canonical with no flip" do
      assert AppearanceArchetype.canonical_facing(:south) == {:south, false}
      assert AppearanceArchetype.canonical_facing(:north) == {:north, false}
      assert AppearanceArchetype.canonical_facing(:east) == {:east, false}
    end
  end
  ...
  describe "sockets/1" do
    test "biped_upright has head and hand sockets for south facing" do
      sockets = AppearanceArchetype.sockets(:biped_upright)
    ...
    test "quadruped has head and tail sockets for east facing" do
      sockets = AppearanceArchetype.sockets(:quadruped)
    ...
    test "returns empty map for unknown archetype" do
      assert AppearanceArchetype.sockets(:unknown_archetype) == %{}
    end
  end
  ...
  describe "layer_order/2" do
    test "biped_upright south: shield comes after torso (in front)" do
      order = AppearanceArchetype.layer_order(:biped_upright, :south)
    ...
    test "biped_upright north: shield comes before torso (behind)" do
      order = AppearanceArchetype.layer_order(:biped_upright, :north)
      torso_idx = Enum.find_index(order, &(&1 == :torso))
      shield_idx = Enum.find_index(order, &(&1 == :shield))
      assert torso_idx != nil
      assert shield_idx != nil
    ...
    test "south and north layer orders differ for biped_upright" do
      south = AppearanceArchetype.layer_order(:biped_upright, :south)
      north = AppearanceArchetype.layer_order(:biped_upright, :north)
      refute south == north
    end

    test "falls back to south order for unknown facing" do
      default = AppearanceArchetype.layer_order(:biped_upright, :southeast)
      south = AppearanceArchetype.layer_order(:biped_upright, :south)
    ...
    test "returns a list for each defined archetype" do
      for archetype <- [:biped_upright, :quadruped, :swarm, :elemental_amorphous, :structure] do
        result = AppearanceArchetype.layer_order(archetype, :south)
  ...
  describe "render_body/2" do
    ...
    test "returns a non-empty SVG string for a generic biped" do
      ...
      result = AppearanceArchetype.render_body(entity, %{})
    ...
    test "biped north render does not show face (no eyes)" do
      ...
      result = AppearanceArchetype.render_body(entity, %{})
      # North facing omits the eye dots (no #333 fill circles used for eyes)
    ...
    test "quadruped (wolf) renders polygon ears" do
      ...
      result = AppearanceArchetype.render_body(entity, appearances)
    ...
    test "entity without :facing field defaults to south" do
      ...
      south_result = AppearanceArchetype.render_body(entity, %{})
      explicit_south = AppearanceArchetype.render_body(Map.put(entity, :facing, :south), %{})
