defmodule GibberingTalesWeb.Engine.SceneServer do
  @moduledoc """
  ...
  alias GibberingTalesWeb.Engine.{State, Rules}
  ...
  def running?(game_id) do
    Registry.lookup(GibberingTalesWeb.GameRegistry, game_id) != []
  end
  ...
  def ensure_started(game_id) do
    case Registry.lookup(GibberingTalesWeb.GameRegistry, game_id) do
      [_] ->
        :ok

      [] ->
        case DynamicSupervisor.start_child(GibberingTalesWeb.SceneSupervisor, {__MODULE__, game_id}) do
          {:ok, _pid} -> :ok
  ...
  defp via(game_id), do: {:via, Registry, {GibberingTalesWeb.GameRegistry, game_id}}
end
