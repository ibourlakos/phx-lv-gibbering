defmodule GibberingTalesWeb.Monitoring.Stores.Local do
  @moduledoc """
  ...
  def handle_info(%EventBatch{state_snapshot: snapshot, events: events}, state) do
    ...
    if Enum.any?(events, &match?(%SessionEnded{}, &1)) do
      ...
      :ets.insert(
        @scene_info_table,
        {campaign_id, map_size(snapshot.actors), GibberingTalesWeb.Engine.State.phase(snapshot)}
      )
  ...
  defp active_campaigns do
    Registry.select(GibberingTalesWeb.GameRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
  end
