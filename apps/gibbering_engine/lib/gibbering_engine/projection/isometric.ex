defmodule GibberingEngine.Projection.Isometric do
  @moduledoc false
  ...
  @behaviour GibberingEngine.Projection
  ...
  @impl GibberingEngine.Projection
  def grid_to_screen(x, y, %{map_w: map_w, map_h: map_h}), do: to_screen(x, y, map_w, map_h)

  @impl GibberingEngine.Projection
  def screen_to_grid(sx, sy, %{map_w: map_w, map_h: map_h}) do
    ox = origin_x(map_w, map_h)
    tw2 = div(@tile_w, 2)
    th2 = div(@tile_h, 2)
    dx = sx - ox
    dy = sy - @origin_y
    x = round((dx / tw2 + dy / th2) / 2)
    y = round((dy / th2 - dx / tw2) / 2)
    {x, y}
  end

  @impl GibberingEngine.Projection
  def origin(map_w, map_h, _opts), do: {origin_x(map_w, map_h), @origin_y}
end
