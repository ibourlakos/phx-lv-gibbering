defmodule GibberingEngine.SpriteCompositorTest do
  ...
  describe "compose/3 — condition_badges layer (badge_renderer injection)" do
    test "no badges by default when badge_renderer not injected" do
      e = Map.merge(entity(), %{conditions: [:prone]})
      result = SpriteCompositor.compose(e, @appearances, show_hp: false, show_body: false)
      refute result =~ ~s(data-layer="condition-badges")
    end

    test "badges included when injected badge_renderer returns non-empty string" do
      renderer = fn _entity -> ~s(<circle cx="4" cy="38" r="4" fill="#d97706"/>) end
      e = entity()
      result = SpriteCompositor.compose(e, @appearances, badge_renderer: renderer)
      assert result =~ ~s(data-layer="condition-badges")
      assert result =~ "#d97706"
    end

    test "badge layer absent when badge_renderer returns empty string" do
      renderer = fn _entity -> "" end
      e = entity()
      result = SpriteCompositor.compose(e, @appearances, badge_renderer: renderer)
      refute result =~ ~s(data-layer="condition-badges")
    end

    test "badge_renderer receives the entity map" do
      parent = self()
      renderer = fn entity -> send(parent, {:got_entity, entity}); "" end
      e = entity()
      SpriteCompositor.compose(e, @appearances, badge_renderer: renderer)
      assert_receive {:got_entity, ^e}
    end
  end
end
