defmodule GibberingWeb.AdminUsersControllerTest do
  ...
  describe "GET /admin/users" do
    test "lists users when authenticated", %{conn: conn} do
      ...
      user = register_user()
      html = conn |> get("/admin/users") |> html_response(200)
    ...
    test "filters by username search", %{conn: conn} do
      ...
      html = conn |> get("/admin/users", %{"search" => unique}) |> html_response(200)
      assert html =~ user.username
  ...
  describe "GET /admin/users/:id" do
    test "shows user detail when authenticated", %{conn: conn} do
      ...
      html = conn |> get("/admin/users/#{user.id}") |> html_response(200)
      assert html =~ user.username
    end
