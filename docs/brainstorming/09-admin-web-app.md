...
| 6 | Content creation UI scope | Separate brainstorm — see [#10](10-content-creation-tools.md). Shared surface: content-type schemas, validation, core editor components. Divergent surface: admin panel shell (platform-scoped, no ownership, immediate publish) vs. game app shell (user-owned, visibility controls, submission/review workflow). |
| 7 | Admin deployment exposure | Same port as the game app for now; auth pipeline is the only gate. Production and QA must restrict `/admin` to internal network/VPN at the reverse proxy. Note added to [docs/dev-setup.md](../dev-setup.md); placeholder environment docs at [docs/qa-setup.md](../qa-setup.md) and [docs/prod-setup.md](../prod-setup.md). Full infra setup deferred to [#62](../issues/062-multi-environment-infra.md). |
| 8 | MFA for support users | Deferred. Record as a requirement in the auth brainstorm when that is opened. |
...
- ~~**Content creation UI scope**: Is the player-facing content creation tool in scope for this brainstorm, or does it get its own?~~ *(decided — separate brainstorm [#10](10-content-creation-tools.md))*
- ~~**Admin deployment exposure**: internal port vs. network-level restriction~~ *(decided — see Decisions table; infra deferred to [#62](../issues/062-multi-environment-infra.md))*
- ~~**MFA for support users**: MVP or deferred?~~ *(deferred — note in auth brainstorm when opened)*
...
| Issue | Title | Notes |
|---|---|---|
| [#64](../issues/064-admin-router-scope-and-pipeline.md) | Admin router scope and pipeline | Prerequisite for all admin features |
| [#65](../issues/065-support-users-schema-and-auth.md) | `support_users` schema, migration, context, and auth | Depends on #64 |
| [#66](../issues/066-support-audit-log.md) | Support audit log | Required by all mutating support actions |
| [#67](../issues/067-admin-crud-users-and-campaigns.md) | Admin CRUD — Users and Campaigns | Depends on #64, #65, #66 |
| [#68](../issues/068-livedashboard-and-campaign-monitoring.md) | LiveDashboard mount + custom campaign monitoring page | Depends on #64 |
| [#69](../issues/069-metrics-store-behaviour-and-local-impl.md) | `MetricsStore` behaviour + `Stores.Local` implementation | Depends on #68 |
| [#70](../issues/070-ugc-content-schema-and-moderation.md) | UGC content schema, `content_trust` flag, and moderation queue | Deferred — blocked on brainstorm #10 |

Previously deferred issues updated by this brainstorm: [#59](../issues/059-character-export-import.md) (export/import design constraints), [#60](../issues/060-umbrella-restructure-for-admin-app.md), [#61](../issues/061-catalogue-entry-versioning.md), [#62](../issues/062-multi-environment-infra.md).
| [#71](../issues/071-admin-catalogue-crud.md) | Admin catalogue CRUD | Deferred — blocked on brainstorm #10 |
